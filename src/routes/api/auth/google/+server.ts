...
export const POST: RequestHandler = async ({ request }) => {
  try {
    ...
    // For development: create a mock user if AUTH_SECRET is not set
    if (!process.env.AUTH_SECRET || process.env.AUTH_SECRET === 'your_32_character_hex_secret_here') {
      console.warn('Using development mock authentication - set AUTH_SECRET in .env for production');
    ...
    if (!googleUserResponse.ok) {
      console.error('Google token verification failed:', await googleUserResponse.text());
    ...
    return json({ error: 'Authentication failed: ' + error.message }, { status: 500 });
  }
};