<script lang="ts">
  ...
  let userInfo = { name: "", picture: "", email: "" };
  let selectedVoiceId = "";
  ...
  if (typeof window !== "undefined") {
    window.addEventListener("storage", (event) => {
      if (
        ...
        event.key === "user_info" ||
        event.key === "selected_voice_id" ||
        event.key === "preferred_transcription_service"
      ) {
        checkAuthState();
      }
    });
  ...
  function checkAuthState() {
    if (typeof window !== "undefined") {
      ...
      const storedTranscriptionService = localStorage.getItem(
        "preferred_transcription_service",
      );
      if (
        storedTranscriptionService &&
        ["groq", "mistral", "auto"].includes(storedTranscriptionService)
      ) {
        preferredTranscriptionService = storedTranscriptionService as
          | "groq"
          | "mistral"
          | "auto";
      }
  ...
  const updateMessageAudio = (
    messageId: number,
    patch: Partial<Pick<Message, "audioUrl" | "audioStatus">>,
  ) => {
  ...
  const pushAssistantError = (text: string) => {
    messages = [
      ...messages,
      createMessage("assistant", text, { includeInContext: false }),
    ];
    isThinking = false;
    scrollToLatest();
  };
  ...
  const updateMessageUserAudioPlaying = (
    messageId: number,
    playing: boolean,
  ) => {
    messages = messages.map((m) =>
      m.id === messageId ? { ...m, userAudioPlaying: playing } : m,
    );
  };
  ...
  const handleResponse = async (res: Response, pendingMessageId?: number) => {
    if (!res.ok) {
      ...
      let message = errorText
        ? `Error communicating with backend: ${errorText}`
          ...
          try {
            ...
            if (data.type === "transcript") {
              ...
              if (accumulatedAudioBytes.length > 0) {
                const totalLength = accumulatedAudioBytes.reduce(
                ...
                updateMessageAudio(assistantMessage.id, {
                  audioUrl: fullUrl,
                  audioStatus: "ready",
                });
      ...
      const res = await fetch(API_ENDPOINT, {
        ...
        body: JSON.stringify({
          ...
          transcription_service:
            preferredTranscriptionService !== "auto"
...
<svelte:head>
  ...
  <meta
    ...
    content="Ami French Tutor - Voice-first French learning with a beautiful lavender orb"
  />
...
<main
  class="flex flex-col h-dvh w-full bg-[#efeae2] font-sans text-slate-900 overflow-hidden relative"
>
  <!-- Top App Bar -->
  <header
    class="h-16 bg-[#f0f2f5] px-4 flex items-center justify-between border-b border-slate-200/60 z-20 shrink-0"
  >
    <div class="flex items-center gap-3 cursor-default">
      ...
      <div class="flex flex-col">
        <h1 class="text-[16px] font-semibold text-slate-800 leading-tight">
          Ami Tutor
        </h1>
        <span
          class="text-[13px] text-emerald-600 font-medium flex items-center gap-1.5"
        >
    ...
    <div class="flex items-center gap-2">
      ...
      <!-- Voice Toggle -->
      <button
        class="flex items-center justify-center w-10 h-10 rounded-full transition-colors {ttsEnabled
  ...
  <!-- Main Chat Area -->
  <div
    class="flex-1 overflow-y-auto px-2 sm:px-4 md:px-10 py-6"
    bind:this={messagesEl}
    style="background-image: radial-gradient(#d5d0c8 1px, transparent 1px); background-size: 24px 24px;"
  >
    <div class="max-w-4xl mx-auto flex flex-col gap-3 pb-4">
      ...
      {#if messages.length === 0}
        <div class="flex justify-center mb-6">
          <div
            class="bg-[#ffeeba] text-slate-800 text-[13px] px-4 py-2 rounded-lg shadow-sm text-center max-w-sm"
          >
            Bonjour ! I am Ami, your French tutor. Messages are end-to-end
            simulated. Send a message or record a voice note to start.
          </div>
        </div>
      {/if}
        ...
        {#if msg.text || msg.role === "user" || msg.pending}
          ...
          >
            <div class="flex flex-col gap-1 max-w-[85%] md:max-w-[70%]">
              <!-- Message Bubble -->
              <div
                class="relative px-3 py-2 sm:px-4 sm:py-2.5 text-[15px] shadow-[0_1px_1px_rgba(0,0,0,0.1)]
              {msg.role === 'user'
              ...
              >
                ...
                {#if msg.role === "user"}
                  <svg
                    class="absolute top-0 -right-2 text-[#d9fdd3]"
                    width="8"
                    height="13"
                    viewBox="0 0 8 13"
                    fill="currentColor"
                ...
                {:else}
                  <svg
                    class="absolute top-0 -left-2 text-white"
                    width="8"
                    height="13"
                    viewBox="0 0 8 13"
                    fill="currentColor"
                ...
                {#if msg.userAudioUrl}
                  ...
                  <div class="flex items-center gap-3 py-1">
                    ...
                    <div
                      class="flex-1 flex flex-col gap-1 min-w-[120px] sm:min-w-[180px]"
                    >
                      <div class="flex items-center gap-1">
                        <!-- Simulated waveform blocks -->
                        <div class="h-5 flex items-center gap-0.5 opacity-60">
                          <div
                            class="w-0.5 h-3 bg-slate-800 rounded-full"
                          ></div>
                          <div
                            class="w-0.5 h-4 bg-slate-800 rounded-full"
                          ></div>
                          <div
                            class="w-0.5 h-2 bg-slate-800 rounded-full"
                          ></div>
                          <div
                            class="w-0.5 h-5 bg-slate-800 rounded-full"
                          ></div>
                          <div
                            class="w-0.5 h-3 bg-slate-800 rounded-full"
                          ></div>
                          ...
                          ></div>
                          <div
                            class="w-0.5 h-1 bg-slate-800 rounded-full"
                          ></div>
                        </div>
                      </div>
                    </div>
                  ...
                  {#if msg.showTranscript}
                    <div
                      class="mt-2.5 pt-2.5 border-t border-emerald-500/10 text-[13.5px] text-slate-700 italic font-normal leading-relaxed"
                    >
                      {#if msg.pending}
                        <span class="flex items-center gap-1.5 text-slate-500">
                          <svg
                            class="animate-spin h-3.5 w-3.5 text-emerald-600"
              ...
              {#if msg.lesson}
                <div
                  class="mt-1 bg-[#f0f2f5] border border-slate-200/60 rounded-lg p-3 shadow-sm text-slate-700 self-start w-full"
                >
                  <div class="flex items-center gap-1.5 mb-1.5">
                    ...
                    >
                    <span
                      class="text-[12px] font-bold uppercase tracking-wider text-slate-500"
                      >Petite leçon</span
                    >
                  </div>
                  <p class="whitespace-pre-wrap text-[13.5px] leading-relaxed">
                    {msg.lesson}
                  </p>
                </div>
              {/if}
      ...
      {#if isThinking}
        <div class="flex justify-start w-full mt-1">
          <div
            class="relative px-4 py-3 bg-white text-slate-900 rounded-lg rounded-tl-none shadow-sm flex items-center gap-1"
          >
            <svg
              class="absolute top-0 -left-2 text-white"
              width="8"
  ...
  <!-- Bottom Input Bar -->
  <footer
    class="bg-[#f0f2f5] px-2 sm:px-4 py-3 border-t border-slate-200/60 shrink-0"
  >
    <div class="max-w-4xl mx-auto">
      ...
      >
        <!-- Text Input -->
        <div
          class="flex-1 bg-white rounded-2xl sm:rounded-full border border-slate-300 shadow-sm flex items-end relative overflow-hidden transition-all focus-within:ring-2 focus-within:ring-emerald-500/20 focus-within:border-emerald-500"
        >
          <textarea
